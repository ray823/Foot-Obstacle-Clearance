...
def main():
    ...
    #model.add(GRU(64, input_shape=(101,29), return_sequences=False))
    #model.add(Dropout(d))
    ...
    print(model.summary())
    with open('./LSTM model performance/Regression_new_early(SGD5000)/modelsummary.txt', 'w') as f:
    ...
    #model.fit(X_train, y_train, nb_epoch = 5000, batch_size=32,validation_data=(X_val, y_val),callbacks=[monitor]) #训练模型1000次
    #epochxxx =  monitor.stopped_epoch+1   #len(model.history.history['loss'])
    #print(epochxxx)    
 

    #保存模型和權重
    saved = model.save('./LSTM model performance/Regression_new_early(SGD5000)/Regression.h5')    
    model.save_weights('./LSTM model performance/Regression_new_early(SGD5000)/Regression_weights.h5')  # to store
